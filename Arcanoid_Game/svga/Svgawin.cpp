...
DLLEXPORT unsigned char *locked_scr;
extern "C" DLLEXPORT int SV_lock(int x,int y,int w,int h) {
  ...
  sv_pitch=sd.lPitch;
  locked_scr=(unsigned char*)sptr;
  if(svga)svga->setbuf(sptr,sv_width,sv_height,sv_pitch);
  if(svga)svga->setcutrect(x,y,x+w-1,y+h-1);
  return 1;
}
...
//  }
  ...
  if(sv_in_window) {
    ...
    if(ddrval!=DD_OK) return 0;
    DDSback->SetColorKey(DDCKEY_DESTBLT,NULL);
    DDSback->SetColorKey(DDCKEY_DESTOVERLAY,NULL);
